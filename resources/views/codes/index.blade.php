                                ...
                                <form action="{{ route('codes.store') }}" method="post">
                                    ...
                                    <div class="form-group">
                                        <input type="text" name="name" class="form-control"
                                            placeholder="اسم كود الخصم" required>
                                    </div>
                                    <div class="form-group">
                                        <input type="number" name="unit_cost" class="form-control"
                                            placeholder="سعر كود الخصم" required>
                                    </div>
                            </div>
                            ...
                            <div class="modal-body">
                                <form id="editForm" method="post">
                                    ...
                                    <div class="form-group text-right">
                                        <label for="is_active">تفعيل</label>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-around">
                                        <h5 class="card-title">{{ $code->name }}
                                        </h5>
                                        <div class="">
                                            @if ($code->is_active === 0)
                                                <span class="badge {{ $code->is_active ? '' : 'badge-danger' }}">
                                                    {{ $code->is_active ? '' : 'غير مفعل' }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="row second-row-links">
                                        <button type="button" class="edit-btn col" data-toggle="modal"
                                            data-target="#editModal" data-code-id="{{ $code->id }}"
                                            data-unit-cost="{{ $code->unit_cost }}"
                                            data-is-active="{{ $code->is_active }}">
                                            تعديل
                                        </button>
    ...
    <script>
        $(document).ready(function() {
            $('.edit-btn').on('click', function() {
                ...
                $('#editModal input[name="is_active"]').checked = 1;
