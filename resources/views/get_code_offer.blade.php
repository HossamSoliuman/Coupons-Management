...
<div class="mt-5">
    <div>
        ...
        <div class="text-center">
            <img src="logo-trans.png" alt="Logo" style="max-width: 250px;" />
        </div>
...
<script>
    ...
    function buildSuccessModal(response) {
        ...
        const alertDiv = $('<div>', { class: 'alert', role: 'alert', style: 'margin-bottom: 0' });

        const title = $('<p>', { class: 'h3 mb-3', text: 'مبروك ربحت ' });
        alertDiv.append(title);

        const name = $('<p>', { class: 'h4 text-bold', style: 'margin-bottom: 0', text: response.success.name });
        alertDiv.append(name);

        const amount = $('<p>', { class: 'h1 text-bold mt-4', text: response.success.amount });
            ...
            error: function (xhr) {
                ...
                $('#modalMessage').html(
                    $('<div>', { class: 'alert', role: 'alert' })
                        .append($('<p>', { class: 'h3', text: xhr.responseJSON.error }))
                );
    ...
    $(document).ready(function () {
        $('#offerForm').submit(function (event) {
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    ...
                    $('#modalMessage').html(
                        $('<div>', { class: 'alert', role: 'alert' })
                            .append($('<p>', { class: 'h3', text: "لم يتم ارسال رسالة التأكيد " }))
                    );
        ...
        $('#otpForm').submit(function (event) {
            event.preventDefault();
            let otp = $('.otp-input').map(function () { return $(this).val(); }).get().join('');
            var phone = $('#phone').val().toLowerCase();
